...
class Arbre :
    ...
    def __fromlist(self,liste) :
        ...
        assert(type(liste)==list or type(liste)==tuple)
        assert(len(liste)>=1)
    ...
    def __repr__(self) :
        if len(self._fils)==0 : return type(self).__name__+"("+repr(self.racine)+")"
        f=[]
        for l in self._fils :
            if l==None : f.append(repr(None))
                ...
                f.append(repr(l))
            pass
        return type(self).__name__+"(("+repr(self.racine)+","+",".join(f)+"))"
